...
from .models import Payment,DatingSubscription
...
class PaymentView(RedirectNotAuthenticatedUserMixin, FormView):
    template_name = 'dating_subscription/Addcard.html'
    form_class = PaymentForm
    success_url = reverse_lazy('payment-success')
    ...
    def get(self, request: HttpRequest, *args: str, **kwargs: Any) -> HttpResponse:
        ...
        if session_plan:
            ...
            return redirect('dating_subscription:subscribe')
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            payment = Payment.objects.create(
                user=self.request.user,
                payment_type='razorpay',
                subscription_plan=DatingSubscription.objects.get(plan_type=self.request.session['selected_plan']['plan_type']),
                razorpay_order_id=razorpay_order['id'],
            ...
            context = {
                ...
                'amount': amount,  # Amount in paise
                'currency': 'INR',
                'callback_url': self.request.build_absolute_uri(reverse_lazy('dating_subscription:payment-callback'))
            }
            return JsonResponse(context)
...
class PaymentDetails(RedirectNotAuthenticatedUserMixin, TemplateView):
    template_name = 'dating_subscription/payment_details.html'
    ...
    def get(self, request, *args, **kwargs):
        ...
        if pay_id:
            ...
            html_string = render_to_string('dating_subscription/invoice_template.html', {'payment_details': payment_details})
...
class SubscriptionView(TemplateView):
    template_name = 'dating_subscription/Subscribe.html'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        subscription_details = DatingSubscription.objects.filter(status='active').order_by('-created_at')
        context['subscription_details'] = subscription_details
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if plan_type:
            try:
                subscription = DatingSubscription.objects.get(plan_type=plan_type)
                # Store the selected plan in session
            ...
            except DatingSubscription.DoesNotExist:
                messages.error(request, "The selected plan does not exist.")
                return redirect('dating_subscription:subscribe')

        return redirect('dating_subscription:payment')

    
class AddPaymentView(TemplateView):
    template_name='dating_subscription/AddPayment.html'


class Invoice(TemplateView):
    template_name = 'dating_subscription/invoice_template.html'
