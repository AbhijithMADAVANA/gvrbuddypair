...
#         # Check if filter_value is 'all' to return all users
#         if filter_value == 'all':
#             queryset = queryset.all()  # Fetch all users
...
class Matches(LoginRequiredMixin, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not isinstance(user, costume_user):
            return redirect('matrimony_home:home')  # URL where non-authenticated users should go
        ...
        try:
            ...
            return redirect('matrimony_home:home') 
